...
app.get('/code/:id',async (req, res) => {
    ...
    try {
        ...
        const [rows] = await connection.execute("SELECT `orden`, `deporte`, `modelo`, `posicion`, `medida`, `lanzador`, `modelo-canasta`, `modelo-palma`, `modelo-logo`, `latigo-dividido`,`latigo`, `pieza-2-dividida`, `pieza-1`, `textura-pieza-1`, `pieza-2`, `textura-pieza-2`, `pieza-2-2`, `textura-pieza-2-2`, `pieza-3`, `textura-pieza-3`, `pieza-4`, `textura-pieza-4`, `pieza-5`, `textura-pieza-5`, `pieza-6`, `textura-pieza-6`, `pieza-7`, `textura-pieza-7`, `pieza-8`, `textura-pieza-8`, `pieza-9`, `textura-pieza-9`, `pieza-10`, `textura-pieza-10`, `pieza-11`, `textura-pieza-11`, `color-canasta`, `detalle-canasta`, `vivos`, `vivos-palma`, `ribete`, `latiguillos`, `hilos`, `correa-lateral`, `correa-interna`, `correa-latigo`, `correa-dedos`, `correa-canasta`, `palma`, `palma-interior`, `contorno-logo`, `logo`, `logo-detalle` FROM gloves WHERE codigo = ?", [userId]);
...
app.get('/code-all/:id',async (req, res) => {
    ...
    try {
        connection = await mysql.createConnection(dbConfig);
        const [rows] = await connection.execute("SELECT * FROM gloves WHERE codigo = ?", [userId]);
...
app.get('/filter/:id',async (req, res) => {
    ...
    try {
        connection = await mysql.createConnection(dbConfig);
        const [rows] = await connection.execute("SELECT * FROM gloves WHERE tag = ?", [userId]);
...
app.get('/seleccion/:id',async (req, res) => {
    const userId = req.params.id;
